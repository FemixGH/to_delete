...
# Тестирование с Confident AI DeepTeam

В проект добавлена интеграция с DeepTeam (https://github.com/confident-ai/deepteam) для запуска дополнительных тестов качества/безопасности против YandexGPT через OpenAI-совместимый прокси.

Компоненты:
- `deepteam_runner.py` — поднимает локальный прокси (`yandex_openai_proxy.py`), запускает DeepTeam с указанными аргументами и собирает артефакты, упаковывая их в единый ZIP-архив.
- Пример конфигурации: `deepteam_tests/sample_config.yaml` — стартовый шаблон, указывающий провайдера `openai` и модель `yandex-gpt` (имя условное; фактическая маршрутизация идёт через локальный прокси). Адаптируйте под нужные пайплайны DeepTeam.

Требования:
- Настроенный доступ к YandexGPT через `.env`/`private-key.pem` (как и для чата).
- Установленный пакет `deepteam` и его зависимости (см. инструкции в репозитории DeepTeam). Если пакет не установлен, `deepteam_runner.py` выведет понятную ошибку и завершится.

Поведение по умолчанию:
- Прокси запускается на `http://127.0.0.1:8001`.
- Runner автоматически проставляет переменные окружения `OPENAI_API_BASE` и `OPENAI_API_KEY` для совместимости клиентов.
- Результаты и логи складываются в `deepteam_runs/run_<timestamp>/` и дополнительно упаковываются в архив `deepteam_report_<timestamp>.zip` в корне проекта (путь можно задать флагом `--zip-path`).

Где искать результаты:
- Логи запуска DeepTeam: `deepteam_runs/run_<timestamp>/deepteam_stdout.txt` и `deepteam_runs/run_<timestamp>/deepteam_stderr.txt`.
- Итоговый архив для скачивания: `deepteam_report_<timestamp>.zip`.

Полезные флаги `deepteam_runner.py`:
- `--proxy-host` / `--proxy-port` — где поднимать OpenAI-прокси.
- `--openai-api-key` — фиктивный ключ для совместимости клиентов OpenAI.
- `--results-root` — корневой каталог для результатов.
- `--zip-path` — явный путь/имя итогового zip-архива.
- После `--` указывайте любые аргументы для `deepteam` (они будут переданы как есть в `python -m deepteam`).

Замечания:
- Убедитесь, что `yandex_openai_proxy.py` не занят другим процессом, если возникают ошибки привязки порта.
- Для специфичных тестов/профилей DeepTeam смотрите официальную документацию проекта.
